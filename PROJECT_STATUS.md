...
Phase 46 production Hosting is deployed from commit `f428b73` at `https://calabash-54fb5.web.app`. The production build uses clean routes while retaining static public data, the current client-side checkout/contact behavior, normal approved-admin tools, and the GitHub Pages rollback artifact. Only Hosting was deployed; Functions, rules, Storage, and Firebase business data remained unchanged. Firebase release `2c8f64` serves bundle `main.70583ccb.js`. The `www.calabashgardens.com` custom-domain mapping is added in Firebase but remains `Needs setup` until Namecheap changes its CNAME from `lscheinhorn.github.io` to `calabash-54fb5.web.app`.
...
Phase 46 Firebase Hosting production is deployed and verified at the default Firebase URL. The live custom domain remains on the released Phase 44 GitHub Pages build until the approved Namecheap CNAME change is saved, Firebase provisions the domain, and the custom-domain routes are verified.
...
- Firebase production Hosting is live at `https://calabash-54fb5.web.app`, while `www.calabashgardens.com` still resolves to the retained GitHub Pages release until the pending Namecheap CNAME cutover. The explicit `npm run deploy` GitHub Pages path remains available as rollback and stays pinned to hash routing.
- Firebase production Hosting sends the reviewed security/cache headers with Content Security Policy in report-only mode. CSP enforcement remains deferred until normal PayPal, EmailJS, Firebase, Storage, preview-frame, and YouTube traffic has been observed without required-resource violations. The still-live GitHub Pages custom domain provides HTTPS/HSTS but no explicit CSP.
...
- 2026-09-04: Added `www.calabashgardens.com` to Firebase Hosting; it remains `Needs setup`. Current Namecheap DNS still routes `www` to `lscheinhorn.github.io` and the apex to GitHub Pages A records. The approved `www` cutover requires replacing that CNAME with `calabash-54fb5.web.app`, then verifying Firebase certificate provisioning and custom-domain routes. GitHub Pages remains intact as rollback.
