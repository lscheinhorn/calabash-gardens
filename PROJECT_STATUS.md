...
Phase 46 production Hosting is live at `https://www.calabashgardens.com` and `https://calabash-54fb5.web.app`. The production build uses clean routes while retaining static public data, the current client-side checkout/contact behavior, normal approved-admin tools, and the GitHub Pages rollback artifact. Only Hosting and the approved `www` DNS mapping changed; Functions, rules, Storage, and Firebase business data remained unchanged. Final Firebase release `d4d514` serves bundle `main.70583ccb.js`. Namecheap now routes `www` to `calabash-54fb5.web.app`, Firebase reports the custom domain `Connected`, and trusted HTTPS and clean customer/admin routes are verified.
...
Phase 46 Firebase Hosting production and custom-domain cutover are complete. `www.calabashgardens.com` now serves the verified Firebase release with clean routes; the prior GitHub Pages deployment remains intact as a DNS rollback target.
...
- Phase 45 Firebase Hosting preview: review the deployed clean routes, headers, responsive/public flows, and read-only admin inspection before deciding whether to prepare a separate DNS/live Hosting cutover. GitHub Pages remains live and retained as rollback.
...
- Phase 37 restricted order-update rules are deployed. The live admin may edit only the approved fulfillment fields; payment, customer, totals, creation, and deletion remain server-owned and denied.
- Phase 38 transactional draft-publish rules are deployed. Drafts created before the transactional baseline must still be discarded, reopened from current live data, and saved again before publishing.
- Phase 40 product-variant and SKU-registry rules are deployed with the matching client. Jetta's supported Inventory save is now the approved initialization path; guarded Functions checkout remains undeployed and disabled.
...
- Active drafts created before Phase 38 do not have a trustworthy live baseline. They must be discarded, reopened from current live data, and saved again before publishing.
...
- The admin and checkout models support product variants, but 72 current production product documents still require Jetta's reviewed starting quantities before they have complete persisted inventory variants. InventoryAdmin synthesizes deterministic complete rows and persists them transactionally; no automatic production migration has run. Guarded server checkout remains disabled and would reject incomplete products.
...
- The live admin build has the Firebase project configuration and still requires an approved admin user to sign in.
...
- Deployed `eventWaitlist` rules bind creates to an existing active, published, future, full, waitlist-enabled event with matching ID/title/date. Rules alone cannot rate-limit anonymous submissions; a server endpoint with abuse protection is still required before broader production use.
...
- Firebase production Hosting is live at both `https://www.calabashgardens.com` and `https://calabash-54fb5.web.app`. The explicit `npm run deploy` GitHub Pages path remains available as rollback and stays pinned to hash routing; rollback requires restoring the prior `www -> lscheinhorn.github.io` CNAME.
- Firebase production Hosting sends the reviewed security/cache headers with Content Security Policy in report-only mode. CSP enforcement remains deferred until normal PayPal, EmailJS, Firebase, Storage, preview-frame, and YouTube traffic has been observed without required-resource violations.
- Independent release review identified a pre-existing Contact failure-feedback mismatch: the EmailJS rejection object is stored directly while the UI's generic error branch checks for the literal `error` string. The Hosting migration does not change this behavior; it remains a separate, explicitly scoped follow-up.
...
- 2026-08-28: Luke approved the controlled Phase 42 release. `main` fast-forwarded from rollback point `ba6ffa0` through reviewed candidate `ab72889`; the pre-release `gh-pages` rollback point was `f59831e`.
- 2026-08-28: Deployed Firestore rules only to `calabash-54fb5`. The CLI compiled and released `firestore.rules`; Functions, Storage rules, Hosting, and production data were excluded.
- 2026-08-28: Pre-deploy review found that the source build omitted the live Pages `CNAME`. Added and independently reviewed `public/CNAME` on `codex/phase42-cname-guard`, fast-forwarded `main` to `2cd3e8f`, and confirmed the build preserves `www.calabashgardens.com`.
- 2026-08-28: Published GitHub Pages commit `e7a744f` with products forced to static and server PayPal, webhook review, and emulator flags forced off. Live Home, Shop, product detail, Cart, Events, Contact, and admin sign-in shell passed; Cart remained `$15 + $17 = $32`, and the past event exposed no purchase controls.
- 2026-08-28: The production no-write SKU check returned `READY FOR INVENTORY ENTRY`: 72 products, 101 variants/SKUs, 0 blockers, and 2 warnings. It performed no inventory or report write.
- Docs checked: updated Phase 41/42 release state, deployment/rollback evidence, the inventory handoff, and current risks. Protected business content/resource files were not edited.
...
- 2026-09-03: Deployed commit `65f1532` only to Firebase Hosting preview channel `phase45-preview` on confirmed existing site/project `calabash-54fb5`. The channel URL is `https://calabash-54fb5--phase45-preview-rs478tis.web.app` and expires 2026-09-10 16:26:10 America/New_York. The CLI uploaded 61 build files and changed no DNS, custom domain, GitHub Pages release, Functions, Firestore/Storage rules, or Firebase business data.
- 2026-09-03: Deployed-preview verification passed direct HTTP 200 responses for Home, Shop, Events, Contact, Cart, a product, Admin, direct admin preview, and unknown SPA paths. Route/HTML responses send `no-cache`; hashed assets send one-year immutable caching; Firebase also sends HTTPS/HSTS and preview `X-Robots-Tag: noindex` alongside the configured report-only CSP and security headers. The live custom domain continued serving the unchanged GitHub Pages `main.778c7ae3.js` bundle.
- 2026-09-03: Browser-controlled deployed-preview verification passed clean internal navigation, direct product refresh, back/forward history, legacy `#/shop` conversion with query preservation, unknown-route recovery, Home/Shop/product/Events/Contact/Cart rendering, disabled EmailJS/PayPal controls, omitted password reset, and generic failed-sign-in feedback. No application errors or CSP violations were observed; the existing cart debug logs remain. Approved-admin sign-in/read-only Firestore inspection still requires Luke or Jetta's real credentials and was not performed by Codex.
...
- 2026-09-04: Phase 46 candidate verification passed 101 regular React/Jest tests with 26 emulator-gated tests skipped, seven Hosting configuration tests, the Functions syntax check, `git diff --check`, both the clean-routed Firebase production build and retained hash-routed GitHub Pages build, and an empty protected-file diff. Both builds retained only the existing unused-code and Browserslist warnings.
- 2026-09-04: Browser-controlled verification of the exact local production artifact passed clean Shop, Cart, Events, Admin, unknown-route, and legacy-hash-route flows. The `$15` Saffron Maple Syrup plus `$17` shipping produced a `$32` cart total; normal PayPal controls rendered; the past 2025 event exposed no purchase action; admin sign-in and password-reset controls rendered; and no browser error or warning logs appeared. No payment, contact, waitlist, password reset, or admin write was submitted.
- 2026-09-04: Deployed commit `f428b73` only to the Firebase Hosting live channel for confirmed project/site `calabash-54fb5`. Firebase release `2c8f64` serves `main.70583ccb.js` at `https://calabash-54fb5.web.app`; no Function, Firestore rule, Storage rule, Firestore business-data, Storage object, or protected-content change was deployed.
- 2026-09-04: Deployed-site verification passed HTTP 200 for Home, Shop, Events, Contact, Cart, product, Admin, and unknown clean routes. HTML sends `no-cache`, hashed assets send one-year immutable caching, and the configured HTTPS/HSTS, COOP, referrer, nosniff, frame, and report-only CSP headers are present. Browser verification passed Shop, Cart, PayPal rendering, past-event gating, and the admin sign-in shell without console warnings or errors.
- 2026-09-04: Added `www.calabashgardens.com` to Firebase Hosting. It initially reported `Needs setup` while Namecheap still routed `www` to `lscheinhorn.github.io`; the apex retained its four GitHub Pages A records. This established the exact CNAME change and rollback boundary before the later verified cutover.
- Docs checked: updated the Phase 46 production Hosting contract, deployment evidence, custom-domain state, rollback boundary, CSP observation period, and known pre-existing Contact error-feedback risk. Protected business content/resource files were not edited.
- 2026-09-04: Pushed the reviewed Phase 46 history through `a5212f3` to `origin/main`, then changed only the Namecheap `www` CNAME from `lscheinhorn.github.io` to `calabash-54fb5.web.app` with TTL left on Automatic. Both Namecheap authoritative servers and Google public DNS returned the Firebase target. The four apex GitHub Pages A records and all other Namecheap settings were left unchanged.
- 2026-09-04: Firebase accepted the custom domain, minted its certificate, and reported `www.calabashgardens.com` as `Connected`. Trusted HTTPS served the exact production bundle `main.70583ccb.js`; Home, Shop, Events, Contact, Cart, Admin, and unknown clean routes returned the SPA artifact with the configured no-cache HTML, immutable hashed-asset, HSTS, COOP, referrer, nosniff, frame, and report-only CSP policy.
- 2026-09-04: Browser-controlled custom-domain verification passed Home, Shop, add-to-cart, the `$15 + $17 = $32` Cart with normal PayPal controls, the past-event no-purchase state, direct `/admin`, and a fresh legacy `#/admin` bookmark conversion to clean `/admin`. The browser console contained only the existing cart debug logs and no warning or error. No payment, contact, waitlist, password reset, sign-in, or admin write was submitted.
- 2026-09-04: One Firebase IPv4 CDN edge retained its pre-connection bare-root 404 after the domain became `Connected`, while cache-busted requests and browser navigation already passed. A second Hosting-only deploy of the same production configuration invalidated that stale entry. Final release `d4d514` serves the unchanged `main.70583ccb.js` bundle, and normal uncached GET and HEAD requests to `https://www.calabashgardens.com/` now return HTTP 200 with the expected headers.
- Docs checked: recorded the completed custom-domain DNS and certificate cutover, exact live release, route/browser evidence, unchanged backend/protected-content boundary, and retained GitHub Pages rollback path. No application or protected business-content/resource file was edited.
...
- `65f1532 feat: add safe firebase hosting preview`
...
- 2026-09-03: Deployed commit `65f1532` to the expiring Firebase Hosting preview channel `phase45-preview` at `https://calabash-54fb5--phase45-preview-rs478tis.web.app` through 2026-09-10 16:26:10 America/New_York. The live custom domain, GitHub Pages artifact, DNS, Functions, rules, Storage objects, and Firestore data were unchanged.
- 2026-09-04: Deployed commit `f428b73` to the Firebase Hosting live channel at `https://calabash-54fb5.web.app` as release `2c8f64`. Only Hosting changed; the custom domain remains on GitHub Pages pending the Namecheap CNAME cutover, and the GitHub Pages deployment remains available as rollback.
- 2026-09-04: Connected `www.calabashgardens.com` to Firebase Hosting by replacing only its Namecheap CNAME with `calabash-54fb5.web.app`. Firebase reports the domain `Connected`; final Hosting release `d4d514` serves trusted HTTPS and verified clean routes after a same-artifact CDN cache-purge redeploy. The prior GitHub Pages release remains intact as the rollback target.
